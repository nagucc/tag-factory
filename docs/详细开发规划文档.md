...
版本依赖关系决定了开发任务的执行顺序。基础架构（1.0）必须优先完成，它是所有其他版本的基础。数据源管理（1.1）完成后才能开发数据对象管理（1.2）。标签基础管理（1.3）相对独立，可以与数据源管理并行开发。标签应用与规则（1.4）需要同时依赖数据对象管理和标签基础管理。后续版本（1.5、1.6）按顺序依赖前置版本。后台用户管理（1.7）相对独立，主要依赖基础认证模块，可以与其他版本并行开发。
